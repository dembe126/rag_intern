...
from langchain_experimental.text_splitter import SemanticChunker    # # Neuer Import für semantisches Chunking
...
from config import EMBEDDING_MODEL_NAME                             # Importiert Konfigurationen
...
def load_document(file_path):
    ...
    # Jetzt: Pro Seite ein Document erstellen
    documents = []
    
    if hasattr(docling_doc, 'pages') and docling_doc.pages:
        for page_num, page in enumerate(docling_doc.pages, 1):
            # Text der einzelnen Seite extrahieren
            try:
                page_text = page.export_to_markdown()
            except:
                # Fallback falls export_to_markdown() nicht funktioniert
                page_text = str(page.text) if hasattr(page, 'text') else ""
            
            # Nur Seiten mit Inhalt hinzufügen
            if page_text.strip():
                documents.append(Document(
                    ...
                    metadata={
                        ...
                        'page_number': page_num,  # ← ECHTE Seitenzahl!
                        'total_pages': len(docling_doc.pages)
                    }
                ))
    else:
        # Fallback: Gesamtes Dokument wenn keine Seiten verfügbar
